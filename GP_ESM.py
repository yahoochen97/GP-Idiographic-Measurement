...
def main():
    ...
    codebook = pd.read_excel("./data/ESM_Codebook.xlsx", index_col=[0])
    item_mapping = dict(zip(codebook.iloc[:,0].to_list(),codebook.iloc[:,1].to_list()))
    reverse_code = codebook.iloc[:,2].to_list()
    item_description = codebook.iloc[:,3].to_list()

    data = pd.read_csv("./data/GP_ESM_cleaned.csv", index_col=[0])
    print(data.head())
    return
